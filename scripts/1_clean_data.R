...
EUI_2019 <- EUI_2019 %>% 
  ...
  mutate(Year = 2019,
         peuro_q3i = case_match(peuro_q3, "1" ~ 1, "2" ~ 2, "3" ~ 3, "4" ~ 4),
         peuro_q3 = case_match(peuro_q3, "1" ~ 1, "2" ~ 0, "3" ~ 0, "4" ~ 0),
         attack = case_when(attack == 5 ~ 3,
                            attack %in% c(1, 2) ~ 1,
                            attack %in% c(3, 4) ~ 2),
...
EUI_2022 <- EUI_2022 %>% 
  select(country, Q7_r_1:Q7_r_16, New_Q5_1, New_Q5_6, Q4, Q5, Q9, Q18_r, Q21, Q24, Q26_r, Q30, Q37, Q40, Q44,
         ...
         Q68_r, Q69, Q70, Q71, Q73, matches("New_Q78_\\d{1,2}$"), weight) %>% 
  select(-Q47a_6) %>% 
  mutate(Year = 2022,
         Q9i = Q9,
         Q9 = case_match(Q9, 1 ~ 1, 2 ~ 0, 3 ~ 0, 4 ~ 0, 999 ~ NA),
